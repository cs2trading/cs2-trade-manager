...
// 定义一个异步函数，用于保存记录页数
const saveRecordPage = async (key:string, type:string, page:number, isEnd:boolean) => {
...
// 定义一个异步函数，用于更新记录页码
export const updateRecordPage = async (page:number, orderType:number, platform:string, isEnd:boolean) => {
  // 获取上一次保存的页码
...
export const updateRecordDate = async (orderType:number, platform:string, date:number) => {
  // 更新点亮的日期
  if (!date) return;
